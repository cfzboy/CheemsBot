...
@register(outgoing=True, pattern=r"^.gmute ?(\d+)?")
...
@register(outgoing=True, pattern=r"^.ungmute ?(\d+)?")
...
CMD_HELP.update({
    "nopm":
    "`.gmute`\
\nUsage: Reply .gmute and it will mute that person in pm  \
\n\n.`ungmute`\
\nUsage:Reply .ungmute and it will unmute that person in pm \
"
})
